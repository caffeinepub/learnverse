...
const readingTexts: Record<Level, ReadingText[]> = {
  okul_oncesi: [
    {
      key: "rf_1",
      title: "Minnoş Kedi",
...
const labels: Record<string, Record<string, string>> = {
  tr: {
    ...
    readyPrompt:
      "Hazır olduğunda 'Başla' butonuna bas ve metni yüksek sesle oku.",
    start: "Başla",
    done: "Bitti",
    result: "Sonuç",
    wpm: "kelime/dakika",
  ...
  es: {
    ...
    readyPrompt:
      "Cuando estés listo, pulsa 'Iniciar' y lee el texto en voz alta.",
    start: "Iniciar",
    done: "Listo",
    result: "Resultado",
    wpm: "palabras/min",
  ...
  fr: {
    ...
    readyPrompt:
      "Quand tu es prêt, appuie sur 'Commencer' et lis le texte à haute voix.",
    start: "Commencer",
    done: "Terminé",
    result: "Résultat",
    wpm: "mots/min",
...
export default function ReadingFluencyPage() {
  ...
  const level: Level = (user?.level as Level) ?? "ilkokul";
  ...
  function finishReading() {
    ...
    if (calculatedWpm >= currentText.targetWPM.min) {
      if (user?.studentNumber) updatePoints(user.studentNumber, 15);
    }
    ...
    const stored = localStorage.getItem("readingFluencyHistory");
    const prev: ReadingRecord[] = stored ? JSON.parse(stored) : [];
  ...
  function getResultEmoji(wpmVal: number) {
  ...
  function getResultMsg(wpmVal: number) {
    const { min, max } = currentText.targetWPM;
  ...
  function getBarColor(wpmVal: number) {
    const { min, max } = currentText.targetWPM;
  ...
  const bestForThisText = history
    .filter((r) => r.key === currentText.key)
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-blue-900 p-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">📖 {lang.title}</h1>
            <p className="text-indigo-200 text-sm">{lang.subtitle}</p>
          </div>
        </div>
        ...
        {phase === "select" && (
          <>
            ...
            <div className="grid gap-4 mb-6">
              {texts.map((t, i) => {
                const best = history
                  .filter((r) => r.key === t.key)
                  .reduce((b, r) => Math.max(b, r.wpm), 0);
                return (
                  <button
                    type="button"
                    key={t.key}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-indigo-200 text-sm">
                            {t.wordCount} {lang.words} · {lang.target}:{" "}
                            {t.targetWPM.min}–{t.targetWPM.max} {lang.wpm}
                          </div>
                        </div>
                      </div>
                      {best > 0 && (
            ...
            {history.length > 0 && (
              <div className="mt-6">
                <h3 className="text-white font-bold mb-3">📊 {lang.history}</h3>
        ...
        {phase === "reading" && (
          <div className="bg-white/10 backdrop-blur rounded-3xl p-6">
            ...
            <div className="text-center mb-4">
              ...
              <div className="text-indigo-200 text-sm">{lang.seconds}</div>
            </div>

            {/* Story text */}
            <div className="bg-white/90 rounded-2xl p-6 mb-6">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-3xl">{currentText.emoji}</span>
        ...
        {phase === "result" && (
          <div className="bg-white/10 backdrop-blur rounded-3xl p-6 text-center">
            <div className="text-6xl mb-3">{getResultEmoji(wpm)}</div>
            <h2 className="text-2xl font-bold text-white mb-1">
              {lang.result}
            </h2>
            <p className="text-indigo-200 mb-6">{getResultMsg(wpm)}</p>
            ...
            <div className="bg-white/20 rounded-2xl p-6 mb-6">
              <div className="flex justify-center gap-8 mb-4">
                <div>
                  ...
                  <div className="text-indigo-200 text-sm">{lang.wpm}</div>
                </div>
                <div className="w-px bg-white/20" />
                <div>
                  <div className="text-indigo-200 text-sm">{lang.target}</div>
                  <div className="text-2xl font-bold text-white">
                    {currentText.targetWPM.min}–{currentText.targetWPM.max}
                  </div>
                  <div className="text-indigo-200 text-sm">{lang.wpm}</div>
                </div>
                <div className="w-px bg-white/20" />
                <div>
                  ...
                  <div className="text-2xl font-bold text-white">{elapsed}</div>
                  <div className="text-indigo-200 text-sm">{lang.seconds}</div>
                </div>
              </div>
              ...
              {bestForThisText > 0 && (
                <div className="mt-2 text-yellow-300 text-sm">
                  {lang.bestWPM}: {bestForThisText} {lang.wpm}
                </div>
              )}
            </div>
