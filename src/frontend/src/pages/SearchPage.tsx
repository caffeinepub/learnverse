...
const ALL_LEVELS: Level[] = ["okul_oncesi", "ilkokul", "ortaokul"];
const PRIMARY_MIDDLE: Level[] = ["ilkokul", "ortaokul"];
const MIDDLE_ONLY: Level[] = ["ortaokul"];

// Compact search index across all content sections
...
export default function SearchPage() {
  ...
  const studentLevel = (profile?.level as Level) || null;
  ...
  const getLabel = (key: string) => {
    ...
    return labels[key]?.[lang] || labels[key]?.tr || key;
  };
  ...
  const get = (obj: Record<string, string>) => obj[lang] || obj.tr;
  ...
  const filteredItems =
    trimmedQuery.length < 2
      ...
      : SEARCH_INDEX.filter((item) => {
          // Filter by level if logged in
          if (studentLevel && !item.levels.includes(studentLevel)) return false;
          const haystack = [item.title, item.category, ...(item.tags || [])]
            .join(" ")
            .toLowerCase();
  ...
  // Group by category
  const grouped = filteredItems.reduce<Record<string, SearchItem[]>>(
    (acc, item) => {
  ...
  const cardBg = isDark
    ...
    : "bg-white border border-slate-100 hover:bg-indigo-50 shadow-sm";

  const textPrimary = isDark ? "text-white" : "text-slate-800";
  const textSecondary = isDark ? "text-white/60" : "text-slate-500";
  const inputBg = isDark
    ? "bg-white/10 border border-white/20 text-white placeholder-white/40 focus:bg-white/15 focus:border-purple-400"
    : "bg-white border border-slate-200 text-slate-800 placeholder-slate-400 focus:border-purple-400 shadow-sm";
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        {trimmedQuery.length < 2 && (
          <div className={`text-center py-12 ${textSecondary}`}>
            ...
            <p className="text-base font-medium">{get(uiText.hint)}</p>
            <p className="text-sm mt-2 opacity-70">
              2+ {lang === "tr" ? "karakter" : "characters"}
            </p>
          </div>
        )}

        {/* No results */}
        {trimmedQuery.length >= 2 && groupEntries.length === 0 && (
          <div
            data-ocid="search.empty_state"
            className={`text-center py-12 ${textSecondary}`}
          >
            <div className="text-6xl mb-4">😔</div>
            <p className="text-lg font-bold">{get(uiText.empty)}</p>
            <p className="text-sm mt-2 opacity-70">
              "<span className={textPrimary}>{query}</span>"
            </p>
          </div>
        )}
                  ...
                  {items.slice(0, 6).map((item, idx) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() => navigate({ to: item.route as "/home" })}
        ...
        {/* Footer */}
        <div className={`mt-10 text-center text-xs ${textSecondary}`}>
