...
export default function ColoringPage() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: drawTemplate is stable
  useEffect(() => {
    drawTemplate(templateIdx);
    // Award points on first use
  ...
  const floodFill = (startX: number, startY: number, fillColor: string) => {
    ...
    const toColorArray = (hex: string): [number, number, number] => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-400 via-purple-400 to-indigo-400">
      ...
      <div className="bg-white/20 backdrop-blur px-4 py-3 flex items-center justify-between">
        ...
        <h1 className="text-white font-black text-xl">🎨 Boyama Sayfası</h1>
        <div className="w-16" />
      </div>

      {pointsAwarded && (
        <div className="mx-4 mt-3 bg-yellow-400/80 text-yellow-900 font-bold text-center rounded-2xl py-2 text-sm">
          🎉 +10 puan kazandın!
        </div>
      )}
      ...
      <div className="p-3 flex flex-col gap-3 max-w-2xl mx-auto">
        ...
        <div className="bg-white/20 backdrop-blur rounded-3xl p-3">
          <div className="text-white font-bold text-sm mb-2">Şablon seç:</div>
          ...
          <div className="flex gap-2 justify-center">
            {(["brush", "eraser", "fill"] as Tool[]).map((t) => (
              <button
                ...
                onClick={() => setTool(t)}
                className={`px-5 py-3 rounded-2xl font-bold text-base transition-all active:scale-95 ${
                  tool === t
          ...
          <div className="flex gap-2 justify-center">
            ...
            {BRUSH_SIZES.map((b) => (
              <button
                ...
                className={`w-12 h-12 rounded-full font-black text-lg transition-all active:scale-95 ${
                  brushSize === b.value
